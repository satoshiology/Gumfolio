...
export default function Dashboard() {
  const { products, loading: productsLoading } = useLiveProductsData();
  const { sales, loading: salesLoading, error } = useLiveSalesData();
  const loading = productsLoading || salesLoading;
  const driftAlerts = useStrategicDrift(products, sales);

  const totalRevenueFormatted = formatPrice(sales.reduce((acc, sale) => acc + sale.price, 0), 'USD');
    ...
    >
      ...
      <motion.div variants={container} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <MetricCard 
          ...
          value={totalSales > 0 ? formatPrice(sales.reduce((acc, s) => acc + s.price, 0) / totalSales, 'USD') : "$0.00"} 
          subValue="Per Sale"
          icon={BarChart3} 
          color="text-tertiary" 
        />
