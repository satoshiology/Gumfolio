...
const CrewCard: React.FC<CrewCardProps> = ({ title, subtitle, allies }) => (
  <div className="bg-zinc-900 border-2 border-yellow-700/50 p-6 rounded-2xl space-y-4 mb-6">
    <div className="flex justify-between items-start">
      <div>
        <h2 className="text-yellow-500 font-headline font-bold text-xl">{title}</h2>
        <p className="text-yellow-600/80 font-body text-sm font-light">{subtitle}</p>
      </div>
    ...
    <div className="flex gap-4">
      {allies.map((ally) => (
        <div key={ally.name} className="flex flex-col items-center gap-1">
          <div className="w-10 h-10 rounded-full bg-zinc-800 text-yellow-500 flex items-center justify-center">
...
export function AlliesPanel() {
  ...
  return (
    <div className="p-6">
      <h1 className="text-yellow-500 font-headline font-extrabold text-3xl mb-8">Good morning, Thierry. Your Crews are ready.</h1>
